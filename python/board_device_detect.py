...
net = load_net("/home/mahxn0/workspace/src/detectAndRecog/src/yolo_surface/data/yolo_watch/watch.cfg", "/home/mahxn0/workspace/src/detectAndRecog/src/yolo_surface/data/yolo_watch/watch.weights", 0)
meta = load_meta("/home/mahxn0/workspace/src/detectAndRecog/src/yolo_surface/data/yolo_watch/watch.data")
...
#video ='rtspsrc location=rtsp://admin:123qweasd@192.168.8.222:554/h264/ch1/main/av_stream latency=0 ! rtph264depay ! h264parse ! omxh264dec ! nvvidconv ! video/x-raw, width=(int)1920, height=(int)1080, format=(string)BGRx ! videoconvert ! appsink'
video='/home/mahxn0/darknet/board1.mp4'
class yolo_surface(object):
    ...
    def detect(self, image, thresh=.1, hier_thresh=.5, nms=.45):
        ...
        arealist=[]
        for seletbox in res:
            temparea=abs(seletbox[5]-seletbox[3])*abs(seletbox[4]-seletbox[2])
        #    print("++++",temparea)
            arealist.append(temparea)
        #   # if temparea>area:
        #   #     area=temparea
        ...
        detectBox=[]
        if arealist:
            nowarea=np.array(arealist)
            res=res[np.argmax(nowarea)]
            detectBox.append(res)
